...
public class BoardWriteController extends HttpServlet {
	...
	protected void doPost(
		 ...
		 * URL로 이동시키기 위해 주로 사용한다. 예를 들어 게시글 쓰기에 대한 요청을
		 * 처리한 후에 Redirect 시키지 않는다면 브라우저의 주소 표시줄에 게시 
		 * 글쓰기에 대한 URL이 그대로 남아 있기 때문에 사용자가 브라우저를 새로
		 * 고침(F5) 하게 되면 바로 이전에 실행된 게시글 쓰기 작업이 반복 실행되어
		 * 중복된 데이터가 DB에 저장되는 문제가 발행할 수 있다. 
		 * 이를 방지하기 위해서 게시글 쓰기가 완료되면 게시글 리스트로 이동시키기
		 * 위해서 response 내장객체의 sendRedirect() 메서드를 사용해 게시글
		 * 리스트의 URL을 웹 브라우저에게 응답하고 웹 브라우저는 응답 받은 URL로
		 * 다시 요청하도록 하는 것이다. 왜 게시글 리스트로 리다이렉트 시켜야 하는가?
		 * 게시글 리스트는 DB에서 데이터를 조회하는 쿼리인 SELECT 쿼리를 사용하기
