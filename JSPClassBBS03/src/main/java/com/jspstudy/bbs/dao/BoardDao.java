...
//	DB에서 현재 페이지를 해당하는 게시글 10개 가져와 반환하는 메서드 (// DB에 등록된 전체 게시글을 읽어와 ArrayList로 반환하는 메서드)
//	현재 페이지 ->
	public ArrayList<Board> boardList(int startRow, int endRow) {
		...
		try {			
			...
			/* 7. 쿼리 실행 결과를 바탕으로 while문 안에서 하나의 게시글을 저장할 
			 * Board 객체를 생성하고 이 객체에 하나의 게시글 정보를 저장하고
...
//	제목, 작성자, 내용에 검색어가 포함된 게시글 리스트를 읽어와 반환하는 메서드
	...
	public Board getBoard(int no) {
		...
		try {
			...
			/* 8. Board 객체를 생성하고 ResultSet으로부터 데이터를 읽어
			 * Board 객체의 각 프로퍼티에 값을 설정한다.
			 * no가 Primary Key 이므로 no에 해당하는 게시글을 DB에서 읽으면
			 * 없거나 또는 하나의 게시글 정보를 얻을 수 있으므로 if문을 사용했다. 
			 * 
			 * ResultSet 객체는 DB로부터 읽어온 데이터에 접근하기 위해 테이블의
			 * 행을 가리키는 cursor를 제공한다. 맨 처음 ResultSet 객체를 반환
			 * 받으면 cursor는 첫 번째 행 바로 이전을 가리키고 있다. ResultSet의
	...
	public void insertBoard(Board board) {
		...
		try {
			...
			/* 7. 데이터베이스에 INSERT 쿼리를 발행하여 게시글 정보를 테이블에 추가한다.
			 *	
	...
	public boolean isPassCheck(int no, String pass) {
		...
		try {
			...
			/* 8. no가 Primary Key 이므로 no에 해당하는 게시글을 DB에서 읽으면
			 * 없거나 또는 하나의 게시글 정보를 얻을 수 있으므로 if문을 사용했다. 
			 * 
			 * ResultSet 객체는 DB로부터 읽어온 데이터에 접근하기 위해 테이블의
			 * 행을 가리키는 cursor를 제공한다. 맨 처음 ResultSet 객체를 반환
		...
		// 11. 메서드 실행 결과로 게시글 비밀번호가 맞는지 여부를 반환한다. 
		return isPass;
	...
	/* 게시글 수정 요청시 호출되는 메서드
	 * 게시글 수정 폼에서 수정하기 버튼이 클릭되면 게시글을 DB에 수정하는 메서드 
	 **/
