...
public class BoardDeleteController extends HttpServlet {
	...
	protected void doPost(
	   ...
	   /* 게시글을 삭제하기 위해서 boardDetail.jsp에서 게시글 삭제 요청을
		* 하면서 테이블에서 게시글 하나를 유일하게 구분할 수 있는 게시글 번호를
		* 요청 파라미터로 보냈기 때문에 이 게시글 번호를 요청 파라미터로부터 읽어
		* BoardDao를 통해서 게시글 번호에 해당하는 게시글을 DB에서 삭제할 수 있다.
		*
		...
		**/	
		...
		// BoardDao의 deleteBoard() 메서드를 이용해 DB에서 게시글을 삭제한다.
		dao.deleteBoard(no);
		 ...
		 * 이동시키기 위해 사용하는 기법이다. 예를 들어 게시글 삭제하기 요청을 처리한
		 * 후 Redirect 시키지 않으면 게시글 삭제 후에 사용자가 새로 고침(F5) 동작을
		 * 하면 바로 이전에 삭제한 게시글과 동일한 게시글을 다시 DB에 삭제하는 작업을 
		 * 하게 되는데 이렇게 되면 없는 게시글을 계속해서 삭제하려고 하는 문제가 발생한다.
		 * 이를 방지하기 위해서 게시글 삭제가 완료되면 게시글 리스트(select 문은 반복
		 * 사용해도 중복된 데이터가 발생하지 않음)로 이동시키기 위해서 response 
		 * 내장객체의 sendRedirect() 메소드를 사용해 게시글 리스트의 URL을
		 * 웹 브라우저에게 응답하고 웹 브라우저는 응답 받은 URL로 다시 요청하도록 하는
		 * 것이다. 이렇게 게시글 삭제와 같이 DB 입력 작업이 연동되는 경우 사용자의
